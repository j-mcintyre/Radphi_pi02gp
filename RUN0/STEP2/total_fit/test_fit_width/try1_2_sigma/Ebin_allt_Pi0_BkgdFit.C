...
void  Ebin_allt_Pi0_BkgdFit() {
   ...
   // Had this section in the "for loop" but I was having possible memory leak error because they kept getting regenerated each iteration.
   TString NEWName;
   ...
   TString htitle;
   int totalhisto;
   for (int tagchan = 0; tagchan < NumEChan; tagchan++) {
      // Left from previous version which included addind |t| binned histos together.
      // For naming png's. Can be replaced with tagchan.
      totalhisto = tagchan; 
      ...
      htitle.Form("Allt_E%d", tagchan);
      remass = (TH1D*)rootf->Get(htitle);

		// Bin width for real data
		double binw = remass->GetBinWidth(1);

		// Histograms maximum heights
		double norm = remass->GetMaximum();

		// Cloning MassBinned histogram for use in different TPads
		TH1F *newhfinal2_1  = (TH1F*)remass->Clone("newhfinal2_1");
		TH1F *newhfinal2_2  = (TH1F*)remass->Clone("newhfinal2_2");
		TH1F *newhfinal2_3  = (TH1F*)remass->Clone("newhfinal2_3");
		TH1F *newhfinal2_4  = (TH1F*)remass->Clone("newhfinal2_4");
		TH1F *newhfinal2_1a = (TH1F*)remass->Clone("newhfinal2_1a");
		TH1F *newhfinal2_2a = (TH1F*)remass->Clone("newhfinal2_2a");
		TH1F *newhfinal2_3a = (TH1F*)remass->Clone("newhfinal2_3a");
		TH1F *newhfinal2_1b = (TH1F*)remass->Clone("newhfinal2_1b");
		TH1F *newhfinal2_1c = (TH1F*)remass->Clone("newhfinal2_1c");
		TH1F *newhfinal2_2b = (TH1F*)remass->Clone("newhfinal2_2b");
		TH1F *newhfinal2_2c = (TH1F*)remass->Clone("newhfinal2_2c");
		TH1F *newhfinal2_3b = (TH1F*)remass->Clone("newhfinal2_3b");
		TH1F *newhfinal2_3c = (TH1F*)remass->Clone("newhfinal2_3c");
		TH1F *newhfinal2_5  = (TH1F*)remass->Clone("newhfinal2_5");
		TH1F *newhfinal2_5a = (TH1F*)remass->Clone("newhfinal2_5a");
		TH1F *newhfinal2_5b = (TH1F*)remass->Clone("newhfinal2_5b");
		TH1F *newhfinal2_6  = (TH1F*)remass->Clone("newhfinal2_6");
		TH1F *newhfinal2_6b = (TH1F*)remass->Clone("newhfinal2_6b");
		TH1F *newhfinal2_6c = (TH1F*)remass->Clone("newhfinal2_6c");
		TH1F *newhfinal2_7  = (TH1F*)remass->Clone("newhfinal2_7");
		TH1F *newhfinal2_8  = (TH1F*)remass->Clone("newhfinal2_8");
		TH1F *newhfinal2_9  = (TH1F*)remass->Clone("newhfinal2_9");
		TH1F *newhfinal2_123  = (TH1F*)remass->Clone("newhfinal2_123");
		TH1F *newhfinal2_321  = (TH1F*)remass->Clone("newhfinal2_321");
		TH1F *newhfinal2_555  = (TH1F*)remass->Clone("newhfinal2_555");
      ...
      // Passing in updated fit parameters
		...
		double GausMax  = fsig1->GetMaximumX();	
      ...
      double GausMax3 = fsig3->GetMaximumX();
		...
		double peak_leak   = ftot->GetParameter(12);   // Leakage "signal"
		double peak2_leak  = ftot->GetParameter(15);   // Leakage "signal"

		double peak_sig3   = ftot->GetParameter(18);   // Eta prime
		...
		double mean_leak   = ftot->GetParameter(13);   // Leakage "signal"
		double mean2_leak  = ftot->GetParameter(16);   // Leakage "signal"

		double mean_sig3   = ftot->GetParameter(19);   // Eta prime
		...
		double sigma_leak  = ftot->GetParameter(14);   // Leakage "signal"
		double sigma2_leak = ftot->GetParameter(17);   // Leakage "signal"

		double sigma_sig3  = ftot->GetParameter(20);   // Eta prime
      ...
      double w1AvgOmega = peak_leak  / (peak_leak + peak2_leak);
      double w2AvgOmega = peak2_leak / (peak_leak + peak2_leak);
      double mu_Omega = (w1AvgOmega * mean_leak) + (w2AvgOmega * mean2_leak);
      double sigma_Omega = sqrt(w1AvgOmega * (pow(sigma_leak, 2.0) + pow((mean_leak - mu_Omega), 2.0)) + w2AvgOmega * (pow(sigma2_leak, 2.0) + pow((mean2_leak - mu_Omega), 2.0)));
      ...
      double val1 = mu_Pi - sigmaRANGE * sigma_Pi;
      double val2 = mu_Pi + sigmaRANGE * sigma_Pi;
      double PercentPiSig = fsig1->Integral(val1, val2) / binw;
      PercentPiSig /= TOTALPi;

      double val3 = mu_Eta - sigmaRANGE * sigma_Eta;
      double val4 = mu_Eta + sigmaRANGE * sigma_Eta;
      double PercentEtaSig = fsig2->Integral(val3, val4) / binw;
      PercentEtaSig /= TOTALEta;

      double val5 = mean_sig3 - sigmaRANGE * sigma_sig3;
      double val6 = mean_sig3 + sigmaRANGE * sigma_sig3;
      double PercentEtapSig = fsig4->Integral(val5, val6) / binw;
      PercentEtapSig /= TOTALEtap;

      //
      double val7 = mu_Pi - (2.0* sigmaRANGE) * sigma_Pi;
      double PercentPiSB1 = fsig1->Integral(val7, val1) / binw;
      PercentPiSB1 /= TOTALPi;
      double otherPiSB1 = fsig1->Integral(0.05, val1) / binw;
      otherPiSB1 /= TOTALPi;

      double val8 = mu_Eta - (2.0* sigmaRANGE) * sigma_Eta;
      double PercentEtaSB1 = fsig2->Integral(val8, val3) / binw;
      PercentEtaSB1 /= TOTALEta;

      double val9 = mean_sig3 - (2.0* sigmaRANGE) * sigma_sig3;
      double PercentEtapSB1 = fsig4->Integral(val9, val5) / binw;
      PercentEtapSB1 /= TOTALEtap;

      //
      double val10 = mu_Pi + (2.0* sigmaRANGE) * sigma_Pi;
      double PercentPiSB2 = fsig1->Integral(val2, val10) / binw;
      PercentPiSB2 /= TOTALPi;

      double val11 = mu_Eta + (2.0* sigmaRANGE) * sigma_Eta;
      double PercentEtaSB2 = fsig2->Integral(val4, val11) / binw;
      PercentEtaSB2 /= TOTALEta;

      double val12 = mean_sig3 + (2.0* sigmaRANGE) * sigma_sig3;
      double PercentEtapSB2 = fsig4->Integral(val6, val12) / binw;
      PercentEtapSB2 /= TOTALEtap;
		...
		TString name123;
		name123.Form("SB1 #rightarrow %.1f to %.1f MeV/c^{2}", val7*1000, val1*1000);

		TString name123sig;
		name123sig.Form("Signal #rightarrow %.1f to %.1f MeV/c^{2}", val1*1000, val2*1000);

		TString name123a;
		name123a.Form("SB2 #rightarrow %.1f to %.1f MeV/c^{2}", val2*1000, val10*1000);

		TString name123b;
		name123b.Form("SB1 = %.2f%% of #pi^{0} Signal", PercentPiSB1 * 100);

		TString name123c;
		name123c.Form("SB1(to 50 MeV) = %.2f%% of #pi^{0} Signal", otherPiSB1 * 100);
      ...
      fsig3->Draw("same");
		...
		TLine *zline321 = new TLine(val3, 0.0, val3, 0.6*normEta);
		zline321->SetLineColor(1);
		zline321->SetLineStyle(9);
		zline321->SetLineWidth(2);
		zline321->Draw("same");

		TLine *zline321a = new TLine(val4, 0.0, val4, 0.6*normEta);
		...
		TLine *zline321b = new TLine(val8, 0.0, val8, 0.28*normEta);
		zline321b->SetLineColor(6);
		zline321b->SetLineStyle(9);
		zline321b->SetLineWidth(2);
		zline321b->Draw("same");

		TLine *zline321c = new TLine(val11, 0.0, val11, 0.28*normEta);
		...
		TString name321;
		name321.Form("SB1 #rightarrow %.1f to %.1f MeV/c^{2}", val8*1000, val3*1000);
		TString name321sig;
		name321sig.Form("Signal #rightarrow %.1f to %.1f MeV/c^{2}", val3*1000, val4*1000);
		TString name321a;
		name321a.Form("SB2 #rightarrow %.1f to %.1f MeV/c^{2}", val4*1000, val11*1000);
		...
		legnd321->Draw("same");
      TString xc321;
      xc321.Form("EtaFit_%d.png", tagchan);
		c321->Print(xc321);
		...
		// Etap Search
		double normEtap = fsig4->Eval(mean_sig3);
      ...
      fsig4->Draw("same");
		...
		TLine *zline555 = new TLine(val5, 0.0, val5, 1.0*normEtap);
		zline555->SetLineColor(1);
		zline555->SetLineStyle(9);
		zline555->SetLineWidth(2);
		zline555->Draw("same");

		TLine *zline555a = new TLine(val6, 0.0, val6, 1.0*normEtap);
		...
		TLine *zline555b = new TLine(val9, 0.0, val9, 0.75*normEtap);
		zline555b->SetLineColor(6);
		zline555b->SetLineStyle(9);
		zline555b->SetLineWidth(2);
		zline555b->Draw("same");

		TLine *zline555c = new TLine(val12, 0.0, val12, 0.75*normEtap);
		zline555c->SetLineColor(6);
		zline555c->SetLineStyle(9);
		zline555c->SetLineWidth(2);
		zline555c->Draw("same");

		TLine *zline555d = new TLine(mean_sig3, 0.0, mean_sig3, 1.25*normEtap);
		...
		TString name555;
		name555.Form("SB1 #rightarrow %.1f to %.1f MeV/c^{2}", val8*1000, val3*1000);
		TString name555sig;
		name555sig.Form("Signal #rightarrow %.1f to %.1f MeV/c^{2}", val3*1000, val4*1000);
		TString name555a;
		name555a.Form("SB2 #rightarrow %.1f to %.1f MeV/c^{2}", val4*1000, val11*1000);

		TString name555b;
		name555b.Form("SB1 = %.2f%% of #eta' Signal", PercentEtapSB1 * 100);

		TString name555c;
		name555c.Form("#eta' Fit Mean = %.1f MeV/c^{2}", mean_sig3 * 1000);
		...
		TString name555g;
		name555g.Form("#eta' Fit #sigma = %.1f MeV/c^{2}", sigma_sig3 * 1000);
		...
		legnd555->Draw("same");
      TString xc555;
      xc555.Form("EtapFit_%d.png", tagchan);
		c555->Print(xc555);




		// These give factors that can be directly plugged into par[].
		double PiPeak_diff1   = (peak_sig1 * peak_sig1)   / norm;
		double PiPeak_diff2   = (peak2_sig1 * peak2_sig1) / norm;
		double EtaPeak_diff1  = (peak_sig2 * peak_sig2)   / norm;
		double EtaPeak_diff2  = (peak2_sig2 * peak2_sig2) / norm;
		double LeakPeak_diff1 = (peak_leak * peak_leak)   / norm;
		double LeakPeak_diff2 = (peak2_leak * peak2_leak) / norm;
		double EtapPeak_diff1 = (peak_sig3 * peak_sig3)   / norm;

      // Calculating the ranges for accidental subtraction weights // 
      // Neutral Pion Regions
		double mx1 = GausMax;  // Maximum of the fit fsig1

      // sigmawidth * half of signal region * 1000 = MeV value of +/- region
      // Round to nearest MeV
      int sigmawidth = lround(1000 * sigmaRANGE * sigma_sig1);
      ...
      // X-values used in integrals of a TF1 fit (don't forget to divide by bin width of real histo)
		double piup     = remass->GetXaxis()->GetBinUpEdge(pimean + sigmaBin);
		double pidown   = remass->GetXaxis()->GetBinLowEdge(pimean - sigmaBin);

		double PiLower  = remass->GetXaxis()->GetBinLowEdge(pimean - 2 * sigmaBin);
		double PiHigher = remass->GetXaxis()->GetBinUpEdge(pimean + 2 * sigmaBin);

		double PiHigh   = remass->GetXaxis()->GetBinLowEdge(pimean + sigmaBin + 1); // So that signal and sideband regions do not overlap
		double PiLow    = remass->GetXaxis()->GetBinUpEdge(pimean - sigmaBin - 1);  // So that signal and sideband regions do not overlap

      cout << endl;
      cout << "PiLower " << PiLower << endl;
      cout << "PiLow " << PiLow << endl;
      cout << "pidown " << pidown << endl;
      cout << "piup " << piup << endl;
      cout << "PiHigh " << PiHigh << endl;
      cout << "PiHigher " << PiHigher << endl;
      cout << endl;
      ...
      TFile *MyFile = new TFile("TwoG_may_12_2023_STEP2.root", "UPDATE");
		...
		if (tagchan == 0) {
         ...
         hdirName.Form("Pi0_SB_Regions_allt_E%d", tagchan);
      ...
      // Eta Regions
		double mx2 = GausMax2;  // ((mean_sig2 * peak_sig2) + (mean2_sig2 * peak2_sig2)) / (peak_sig2 + peak2_sig2);

      // sigmawidthEta * half of signal region * 1000 = MeV value of +/- region
      // Round to nearest MeV
      int sigmawidthEta = lround(1000 * sigmaRANGE * sigma_sig2);

      // See if it is close to the next bin
      int AddBinEta = 0;
      if (sigmawidthEta % 5 >= 3) {
         AddBinEta += 1;
      }
      ...
      // X-values used in integrals of a TF1 fit (don't forget to divide by bin width of real histo)
		...
		double EtaLower  = remass->GetXaxis()->GetBinLowEdge(etamean - 2 * sigmaBinEta);
		double EtaHigher = remass->GetXaxis()->GetBinUpEdge(etamean + 2 * sigmaBinEta);

		double EtaHigh   = remass->GetXaxis()->GetBinLowEdge(etamean + sigmaBinEta + 1); // So that signal and sideband regions do not overlap
		double EtaLow    = remass->GetXaxis()->GetBinUpEdge(etamean - sigmaBinEta - 1);  // So that signal and sideband regions do not overlap

      cout << endl;
      cout << "EtaLower " << EtaLower << endl;
      cout << "EtaLow " << EtaLow << endl;
      cout << "etadown " << etadown << endl;
      cout << "etaup " << etaup << endl;
      cout << "EtaHigh " << EtaHigh << endl;
      cout << "EtaHigher " << EtaHigher << endl;
      cout << endl;


      // Eta Regions
		double mx3 = GausMax4;  // ((mean_sig2 * peak_sig2) + (mean2_sig2 * peak2_sig2)) / (peak_sig2 + peak2_sig2);

      // sigmawidthEta * half of signal region * 1000 = MeV value of +/- region
      // Round to nearest MeV
      int sigmawidthEtap = lround(1000 * sigmaRANGE * sigma_sig3);

      // See if it is close to the next bin
      int AddBinEtap = 0;
      if (sigmawidthEtap % 5 >= 3) {
         AddBinEtap += 1;
      }
      ...
      int etapmean      = remass->FindBin(mx3);
      ...
      // X-values used in integrals of a TF1 fit (don't forget to divide by bin width of real histo)
		...
		double EtapLower  = remass->GetXaxis()->GetBinLowEdge(etapmean - 2 * sigmaBinEtap);
		double EtapHigher = remass->GetXaxis()->GetBinUpEdge(etapmean + 2 * sigmaBinEtap);

		double EtapHigh   = remass->GetXaxis()->GetBinLowEdge(etapmean + sigmaBinEtap + 1); // So that signal and sideband regions do not overlap
		double EtapLow    = remass->GetXaxis()->GetBinUpEdge(etapmean - sigmaBinEtap - 1);  // So that signal and sideband regions do not overlap

      cout << endl;
      cout << "EtapLower " << EtapLower << endl;
      cout << "EtapLow " << EtapLow << endl;
      cout << "etapdown " << etapdown << endl;
      cout << "etapup " << etapup << endl;
      cout << "EtapHigh " << EtapHigh << endl;
      cout << "EtapHigher " << EtapHigher << endl;
      cout << endl;
      ...
      ///////////////////////////////////////////////////////////
		...
		newhfinal2_1->GetXaxis()->SetRangeUser(0.05, 0.24);
		fback->Draw("same");
		ftot->Draw("same");

		TLegend *legnd5 = new TLegend(0.6,0.68,0.89,0.89);
		//legnd5->SetHeader("The Legend Title","C");   // option "C" allows to center the header
		...
		legnd5->Draw("same");
		//c1->cd(1)->Print("c1_1fit_TwoG_may_12_2023.png");
		...
		newhfinal2_1b->SetMinimum(0);
		newhfinal2_1b->GetXaxis()->SetRangeUser(0.05, 0.24);
      ...
      TString NEWtitle;
		...
		TLine *zeroline2 = new TLine(0.3, 0, 0.8, 0);
		zeroline2->SetLineColor(1);
		zeroline2->SetLineStyle(10);
		zeroline2->SetLineWidth(2);
		zeroline2->Draw("same");

		TLegend *legnd5b = new TLegend(0.6,0.58,0.89,0.89); // (0.6,0.58,0.89,0.89);
		legnd5b->SetTextSize(0.028);
		legnd5b->AddEntry(newhfinal2_1b,"Real Data","f");
		//legnd5b->AddEntry(ftot, "Signal & Background Fit","l");
		legnd5b->AddEntry(fsig1,"#pi^{0} Signal Fit","l");
		legnd5b->AddEntry(fback,"Background Fit","l");
		legnd5b->AddEntry(zerolineA,"2.5#sigma Signal Region Boundary","l");
		legnd5b->AddEntry(zerolineC,"5#sigma Bkgd Region Boundary","l");
		legnd5b->Draw("same");

		//c1->cd(2)->Print("c1_2fit_TwoG_may_12_2023.png");


		c1->cd(3);
		newhfinal2_1a->GetXaxis()->SetRangeUser(0.05, 0.24);
		...
		TLine *zeroline = new TLine(0.05, 0, 0.24, 0);
		zeroline->SetLineColor(1);
		zeroline->SetLineStyle(10);
		zeroline->SetLineWidth(2);
		zeroline->Draw("same");
		//c1->cd(3)->Print("c1_3fit_TwoG_may_12_2023.png");


		c1->cd(4);
		gStyle->SetOptFit(1100);
		newhfinal2_1c->GetXaxis()->SetRangeUser(0.05, 0.24);
		newhfinal2_1c->SetTitle("#pi^{0} Signal Fit");
		//double mini1 = -0.05 * norm;
		//newhfinal2_1c->SetMinimum(mini1);
		newhfinal2_1c->SetLineColor(0);
		//newhfinal2_1c->SetFillColor(18);
		newhfinal2_1c->Draw("hist");
		fsig1->Draw("same");

		TLine *zerolineD2DD = new TLine(GausMax, 0, GausMax, 1.03*norm);
		zerolineD2DD->SetLineColor(6);
		zerolineD2DD->SetLineStyle(9);
		zerolineD2DD->SetLineWidth(2);
		zerolineD2DD->Draw("same");

		double pidiff = (abs(((GausMax*1000)-134.9766)/134.9766))*100;
		TString titlepi;
		TString namepi;
		namepi.Form("#pi^{0} Fit Peak = %.1f MeV/c^{2}", GausMax*1000);
		titlepi.Form("Difference = %.2f %%", pidiff);

		TLegend *legnd5a = new TLegend(0.6,0.68,0.89,0.89);
		legnd5a->SetTextSize(0.028);
		//legnd5a->AddEntry(newhfinal2_1c,"Real Data","f");
		...
		legnd5a->Draw("same");
		//c1->cd(4)->Print("c1_4fit_TwoG_may_12_2023.png");
		char histo1[20];
		...
		double normE = fsig2->Eval((mean_sig2 + mean2_sig2)/2);
		gStyle->SetOptStat(0);
      ...
      fsig3->Draw("same");
		...
		legnd4->Draw("same");
		//c2->cd(1)->Print("c2_1fit_TwoG_may_12_2023.png");
      ...
      fsig3->Draw("same");
		...
		TLine *zerolineA1 = new TLine(etadown, 0, etadown, 0.6*normE);
		zerolineA1->SetLineColor(1);
		zerolineA1->SetLineStyle(9);
		zerolineA1->SetLineWidth(2);
		zerolineA1->Draw("same");

		TLine *zerolineB1 = new TLine(etaup, 0, etaup, 0.6*normE);
		zerolineB1->SetLineColor(1);
		zerolineB1->SetLineStyle(9);
		zerolineB1->SetLineWidth(2);
		zerolineB1->Draw("same");

		TLine *zerolineC1 = new TLine(EtaLower, 0, EtaLower, 0.25*normE);
		zerolineC1->SetLineColor(6);
		zerolineC1->SetLineStyle(9);
		zerolineC1->SetLineWidth(2);
		zerolineC1->Draw("same");

		TLine *zerolineD1 = new TLine(EtaHigher, 0, EtaHigher, 0.25*normE);
		zerolineD1->SetLineColor(6);
		zerolineD1->SetLineStyle(9);
		zerolineD1->SetLineWidth(2);
		zerolineD1->Draw("same");
		...
		legnd5c->AddEntry(zerolineA1,"2#sigma Signal Region Boundary","l");
		legnd5c->AddEntry(zerolineC1,"4#sigma Bkgd Region Boundary","l");
		legnd5c->Draw("same");
		//c2->cd(2)->Print("c2_2fit_TwoG_may_12_2023.png");
      ...
      fsig3->Draw("same");
		...
		//c2->cd(3)->Print("c2_3fit_TwoG_may_12_2023.png");
		...
		newhfinal2_2c->GetXaxis()->SetRangeUser(0.33, 0.77);
		newhfinal2_2c->SetTitle("#eta Signal Fit");
		newhfinal2_2c->SetMinimum(0);
		...
		//c2->cd(4)->Print("c2_4fit_TwoG_may_12_2023.png");
      ...
      fsig3->Draw("same");
		...
		legnd->Draw("same");
		//c3->cd(1)->Print("c3_1fit_TwoG_may_12_2023.png");


		c3->cd(2);
		double normE3 = ftot->Eval(mean_sig3);
      ...
      fsig3->Draw("same");
		...
		/*
		TLine *zerolineA2 = new TLine(etapdown, 0, etapdown, 1.6*normE3);
		zerolineA2->SetLineColor(1);
		zerolineA2->SetLineStyle(9);
		zerolineA2->SetLineWidth(2);
		zerolineA2->Draw("same");

		TLine *zerolineB2 = new TLine(etapup, 0, etapup, 1.6*normE3);
		zerolineB2->SetLineColor(1);
		zerolineB2->SetLineStyle(9);
		zerolineB2->SetLineWidth(2);
		zerolineB2->Draw("same");

		TLine *zerolineC2 = new TLine(EtapLower, 0, EtapLower, 1.6*normE3);
		zerolineC2->SetLineColor(6);
		zerolineC2->SetLineStyle(9);
		zerolineC2->SetLineWidth(2);
		zerolineC2->Draw("same");

		TLine *zerolineD2 = new TLine(EtapHigher, 0, EtapHigher, 1.6*normE3);
		...
		*/
		...
		TLegend *legndz = new TLegend(0.36,0.74,0.64,0.89); // (0.57,0.68,0.89,0.89);
		...
		//legndz->AddEntry(zerolineA2,"2#sigma Signal Region Boundary","l");
		//legndz->AddEntry(zerolineC2,"4#sigma Bkgd Region Boundary","l");
		legndz->Draw("same");

		//c3->cd(2)->Print("c3_2fit_TwoG_may_12_2023.png");
      ...
      fsig3->Draw("same");
		...
		newhfinal2_3c->SetMinimum(0);
		double gryf = fsig4->Eval(mean_sig3);
		newhfinal2_3c->SetMaximum(gryf*1.05);
		fsig4->Draw("same");

		TLine *zerolineD2 = new TLine(GausMax4, 0, GausMax4, 1.03*gryf);
		zerolineD2->SetLineColor(6);
		zerolineD2->SetLineStyle(9);
		zerolineD2->SetLineWidth(2);
		zerolineD2->Draw("same");
		...
		//c3->cd(4)->Print("c3_4fit_TwoG_may_12_2023.png");
      ...
      ftot->Draw("same");
      TString histo10;
      histo10.Form("M2g_E%d_AllSignals.png", tagchan);
      c11->Print(histo10);
		...
		cout << "ndofTot -> " << ndofTot << endl;
		cout << "fit probabilityTot -> " << probTot << endl; 
      ...
      TString Bkhtitle;
      Bkhtitle.Form("Pi0_Bkgd_parameters_allt_E%d", tagchan);
		...
		if (output.is_open()) {
			output << "Histogram = " << htitle << endl;
         if (tagchan == 0) {
            output << "Tagger Channel Range = " << totalhisto << "  -->  E_photon = " << Range0b << " to " << Range0a << " GeV" << endl;
         }
         else if (tagchan == 1) {
            output << "Tagger Channel Range = " << totalhisto << "  -->  E_photon = " << Range1b << " to " << Range1a << " GeV" << endl;
         }
         else if (tagchan == 2) {
            output << "Tagger Channel Range = " << totalhisto << "  -->  E_photon = " << Range2b << " to " << Range2a << " GeV" << endl;
         }
         else {
            output << "Tagger Channel Range = " << totalhisto << "  -->  E_photon = " << Range3b << " to " << Range3a << " GeV" << endl;
         }
			output << "|t| = 0.1 to 1.9 GeV^2/c^4 -- Using XPi_bins[20][2]" << endl;
			output << endl;
         ...
         output << "Status = " << statname << endl;
         output << endl;
         output << "chisqTot -> " << chisqTot << endl;
         output << "ndofTot -> " << ndofTot << endl;
         output << "fit probabilityTot -> " << probTot << endl;
         output << endl;

			output << "Pi0 fit peak           = " << GausMax*1000 << " MeV/c^2"  << endl;
         output << "PiPar = " << PiPar[1] << endl;
			output << "Eta fit peak           = " << GausMax2*1000 << " MeV/c^2"  << endl;
         output << "EtaPar = " << EtaPar[1] << endl;
			output << "Omega Leakage fit peak = " << GausMax3*1000 << " MeV/c^2"  << endl;
         output << "OmegaPar = " << OmegaPar[1] << endl;
			output << "Etap fit peak          = " << GausMax4*1000 << " MeV/c^2"  << endl;
			output << endl;
         ...
         if (((mean_leak - 0.00000001) < dparlim13) || ((mean_leak + 0.00000001) > uparlim13)) {
            output << "  ****";
         }
         output << endl;
         output << "3G_leak_sigma1 -> " << ftot->GetParameter(14);
         if (((sigma_leak - 0.00000001) < dparlim14) || ((sigma_leak + 0.00000001) > uparlim14)) {
            output << "  ****";
         }
         output << endl;
         output << "3G_leak_mean_2 -> " << ftot->GetParameter(16);
         if (((mean2_leak - 0.00000001) < dparlim16) || ((mean2_leak + 0.00000001) > uparlim16)) {
            output << "  ****";
         }
         output << endl;
         output << "3G_leak_sigma2 -> " << ftot->GetParameter(17);
         if (((sigma2_leak - 0.00000001) < dparlim17) || ((sigma2_leak + 0.00000001) > uparlim17)) {
            output << "  ****";
         }
         output << endl;
         output << endl;
         ...
         if (((mean_sig3 - 0.00000001) < dparlim19) || ((mean_sig3 + 0.00000001) > uparlim19)) {
            output << "  ****";
         }
         output << endl;
         output << "Etap___sigma_1 -> " << ftot->GetParameter(20);
         if (((sigma_sig3 - 0.00000001) < dparlim20) || ((sigma_sig3 + 0.00000001) > uparlim20)) {
            output << "  ****";
         }
         output << endl;
         output << endl;
         output << "p0     -> " << ftot->GetParameter(21) << endl;
         output << "p1     -> " << ftot->GetParameter(22) << endl;
         output << "p2     -> " << ftot->GetParameter(23) << endl;
         output << "p3     -> " << ftot->GetParameter(24) << endl;

			output << endl;
			output << "   // Energy Rescaling Factor" << endl;
			output << "ER = 0.97;" << endl; 
			output << "0.1 <= |t| <= 1.9 GeV^2/c^4" << endl;
			output << endl;

			output << "   // Tagging Scaling Factor" << endl;
			output << "TSF = 0.935198 +/- 0.00249013;" << endl; 
			output << endl;

			output << "bin width -> " << binw << endl;
			output << endl;

         output << "PiLower  = " << PiLower << endl;
         output << "PiLow    = " << PiLow << endl;
         output << "pidown   = " << pidown << endl;
         output << "piup     = " << piup << endl;
         output << "PiHigh   = " << PiHigh << endl;
         output << "PiHigher = " << PiHigher << endl;

			output << "_____________________________________________" << endl;
			output << endl;
		}
		else {
